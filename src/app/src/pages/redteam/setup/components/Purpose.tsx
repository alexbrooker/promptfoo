...
import {
  generateRedTeamConfigFromOnboarding,
  isOnboardingDataComplete,
} from '@app/utils/autoRedTeamConfig';
...
export default function Purpose({ onNext }: PromptsProps) {
  ...
  const autoGenerateFromOnboarding = () => {
    recordEvent('feature_used', {
      feature: 'redteam_config_auto_generate',
      industry: onboardingData.industry,
      role: onboardingData.chatbotRole,
    });
    const autoConfig = generateRedTeamConfigFromOnboarding(onboardingData);
    setTestMode('application');
    setFullConfig(autoConfig);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Dialog open={autoGenerateDialogOpen} onClose={() => setAutoGenerateDialogOpen(false)}>
        <DialogTitle>Auto-Generate Red Team Configuration?</DialogTitle>
        <DialogContent>
          <Typography sx={{ mb: 2 }}>
            Generate a customized red team configuration based on your profile information:
          </Typography>
          <Box sx={{ ml: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              • <strong>AI Role:</strong> {onboardingData.chatbotRole}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              • <strong>Company:</strong> {onboardingData.company}
            </Typography>
            <Typography variant="body2" color="text.secondary">
              • <strong>Use Case:</strong> {onboardingData.useCase?.substring(0, 100)}
              {onboardingData.useCase && onboardingData.useCase.length > 100 ? '...' : ''}
            </Typography>
          </Box>
          <Typography sx={{ mt: 2 }}>
            This will automatically configure appropriate security tests, plugins, and application
            details. Current settings will be replaced.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setAutoGenerateDialogOpen(false)}>Cancel</Button>
          <Button
            onClick={autoGenerateFromOnboarding}
            variant="contained"
            color="primary"
        ...
        <Stack direction="column" spacing={4}>
          ...
          >
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              {isOnboardingDataComplete(onboardingData) && (
                <Button
                  variant="contained"
          ...
          {!isOnboardingDataComplete(onboardingData) && (
            ...
            >
              <Typography variant="body2">
                <strong>💡 Pro Tip:</strong> Complete your profile information in onboarding to
                enable automatic red team configuration generation based on your industry and AI use
                case.
              </Typography>
            </Alert>
          )}
